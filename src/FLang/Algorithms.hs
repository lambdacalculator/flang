...
glushkov r = (ss, d_final, fs) where
  (_, xs, b, d_final) = rcat r 1 [0] (\_ _ -> [])
  
  ss = if b then 0 : xs else xs
  fs q = q == 0
  ...
  rcat (Let c) j ss' d' = (j + 1, [j], False, d'') where
    d'' q a = if q == j && a == c then ss' else d' q a
  rcat (Union r1 r2) j ss' d' = (n1, xs2 ++ xs1, b1 || b2, d'') where
    (n2, xs2, b2, d2) = rcat r2 j ss' d'
    (n1, xs1, b1, d1) = rcat r1 n2 ss' d2
    d'' q a = if q < n2 then d2 q a else d1 q a
  rcat (Cat r1 r2) j ss' d' = (n1, xs_final, b1 && b2, d'') where
    (n2, xs2, b2, d2) = rcat r2 j ss' d'
    ss2 = if b2 then merge xs2 ss' else xs2
    (n1, xs1, b1, d1) = rcat r1 n2 ss2 d2
    xs_final = if b1 then xs2 ++ xs1 else xs1
    d'' q a = if q < n2 then d2 q a else d1 q a
